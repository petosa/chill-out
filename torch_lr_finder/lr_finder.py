...
class LRFinder(object):
    ...
    def __init__(self, model, optimizer, criterion, device=None, memory_cache=True, cache_dir=None, verbose=True):
        self.model = model
    ...
    def range_test(
        ...
        bar = tqdm if self.verbose else lambda x : x
        for iteration in bar(range(num_iter)):
            ...
            if loss > diverge_th * self.best_loss:
                if self.verbose: print("Stopping early, the loss has diverged")
                break

        if self.verbose: print("Learning rate search finished. See the graph with {finder_name}.plot()")
